...
void setup() {
  ...
  EEPROM.begin(512);                                              // This is needed for ESP8266
}
...
void writeEeprom(int addressToWriteTo) {
  if(wasWriteSuccess == false) {
    ...
    if(EEPROM.commit()) {                                         // If commit is successful (only works with ESP8266):
      ...
      Serial.println("EEPROM Write failed");                      // If writing to EEPROM failed, print error message
    }
  }
}
